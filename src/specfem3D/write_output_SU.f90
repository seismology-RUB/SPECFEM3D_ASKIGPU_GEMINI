...
!=====================================================================
  ...
  subroutine write_output_SU(all_seismograms,nrec_store,istore,seismo_path)

! writes out seismograms in SU (Seismic Unix) format

  use constants, only: CUSTOM_REAL,MAX_STRING_LEN,IMAIN,NDIM,NB_RUNS_ACOUSTIC_GPU, &
  ...
  open(unit=IIN_SU1,file=trim(seismo_path)//'/output_list_stations.txt',status='old',iostat=ier)
  ...
  open(unit=IIN_SU1,file=trim(seismo_path)//'/output_list_sources.txt',status='old',iostat=ier)
  ...
  final_LOCAL_PATH = seismo_path(1:len_trim(seismo_path)) // '/'
