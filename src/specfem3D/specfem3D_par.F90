...
!-----------------------------------------------------------------
  ...
  ! for couple with external code : DSM and AxiSEM (added by VM) for the moment and GEMINI (added by WF)
  ...
  ! for coupling with external code : GEMINI (added by WF)
  integer(kind=8) :: GEMINI_HDF_FILE_ID,HDF_XFERPRP
  real(kind=CUSTOM_REAL) :: GEMINI_DT,GEMINI_TLEN
  real(kind=CUSTOM_REAL), dimension(6) :: GEMINI_INJECTION_TIMING
  real(kind=CUSTOM_REAL), dimension(3) :: GEMINI_INJECTION_EVLOC
!  real, dimension(:,:,:), pointer :: veltrac_all
  real, dimension(:,:), pointer :: veltrac1 => null(),veltrac2 => null()
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: veltrac1_cuda, veltrac2_cuda
...
!-----------------------------------------------------------------------
!  multiple sequential sources (added by WF)
!-----------------------------------------------------------------------
  integer :: num_sequential_sources
  character(len=MAX_STRING_LEN), dimension(:), allocatable :: sequential_sources_description
  logical :: ANOTHER_SEQUENTIAL_SOURCE
  integer :: current_sequential_source
  integer :: SEQUENTIAL_SOURCES_MODE  ! either 1:cmt, 2:force or 3:injection
...
!-----------------------------------------------------------------
  ...
  ! real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: sourcearray   ! not used globally
...
!-----------------------------------------------------------------
  ...
  double precision :: time_specfem_init
...
! parameters for generating ASKI output
  ...
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: ASKI_efactors_re,ASKI_efactors_im
  ...
  real, dimension(:,:,:), allocatable :: ASKI_spectra_local_single_re,ASKI_spectra_local_single_im
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: ASKI_local_disp,ASKI_local_strain
  ...
  real, dimension(:), allocatable :: ASKI_phase_end_time
  double precision, dimension(:), allocatable :: ASKI_xg,ASKI_yg,ASKI_zg
  logical :: ASKI_store_veloc
  type (raytable_travel_times) :: ASKI_ttime_table
...
! is also done on GPU (no effect if GPU_MODE = .false.)
  logical :: ASKI_USES_GPU
...
! the main file needs to be written only once and for all. When the models are path specific the main file should be
! written for each event.
  logical :: ASKI_MAIN_FILE_WRITE

! name of the ASKI main path
  character(len=500) :: ASKI_MAIN_PATH

! id of ASKI output (e.g. eventID, station name plus component, etc.)
  integer, parameter :: length_ASKI_output_ID = 17
...
! File with travel time table to taper synthetic seismograms at end based on travel time and phase window length
!
  character(len=500) :: ASKI_TTIME_TABLE_FILE

! parameter COMPUTE_PHASE_END_TIMES controls if phase end times for displacements and Green tensors are computed
  logical :: COMPUTE_PHASE_END_TIMES

! name of the path to phase end times
  character(len=500) :: PATH_PHASE_END_TIMES
...
! Decide whether the time series should be tapered by a hanning taper (at the end of the phase window)
! before (i.e. on-the-fly while) applying the discrete Fourier transform.
!
  logical :: ASKI_DFT_apply_taper
  integer :: ASKI_DFT_ntaper_length                                       ! length in Specfem samples
  double precision :: ASKI_DFT_taper_length                               ! length in seconds
  integer, dimension(:), allocatable :: ASKI_DFT_ntaper_start             ! Specfem sample of taper start
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: ASKI_DFT_taper_values    ! taper values
...
!------------------------------------------------------------------------------------
! Spherical box center coordinates (degrees)
  double precision :: ASKI_lat_box_center, ASKI_lon_box_center
! Earth radius
  double precision :: ASKI_rearth
