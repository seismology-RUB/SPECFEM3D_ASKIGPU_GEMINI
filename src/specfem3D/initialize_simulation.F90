...
!=====================================================================
  ...
  ! read the task file if desired
  if (MULTIPLE_SEQUENTIAL_SOURCES) then
    call read_sequential_sources_description(BROADCAST_AFTER_READ)
  else
    num_sequential_sources = 1
  ...
  if (.level.errmsg == 2) then
    call exit_mpi(myrank,'problems opening HDF environment')
  endif
  call setXferprpIndependentHDFWrapper(HDF_XFERPRP,errmsg)
  if (.level.errmsg == 2) then
   call exit_mpi(myrank,'problems setting HDF xfer property')
  endif
...
!
  ...
  if (MULTIPLE_SEQUENTIAL_SOURCES) then
    if (SIMULATION_TYPE /= 1) then
      call exit_mpi(myrank,'multiple sequential sources only for simulation type = 1')
    endif
    if (USE_EXTERNAL_SOURCE_FILE) then
      call exit_MPI_without_rank("In MULIPLE_SEQUENTIAL_SOURCES mode, USE_EXTERNAL_SOURCE_FILE is not supported yet")
    endif
    if (NSOURCES > 1) then
      call exit_MPI_without_rank("In MULIPLE_SEQUENTIAL_SOURCES mode, NSOURCES should be 1")
    endif
    if (COUPLE_WITH_INJECTION_TECHNIQUE .and. (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_FK)) then
      call exit_MPI_without_rank("In MULIPLE_SEQUENTIAL_SOURCES mode, no FK injection supported")
    endif
    if (GRAVITY) then
      call exit_MPI_without_rank("In MULIPLE_SEQUENTIAL_SOURCES mode, gravity not yet supported")
    endif
    if (NOISE_TOMOGRAPHY > 0) then
      call exit_MPI_without_rank("In MULIPLE_SEQUENTIAL_SOURCES mode, noise tomography not yet supported")
    endif
    if (FAULT_SIMULATION) then
      call exit_MPI_without_rank("In MULIPLE_SEQUENTIAL_SOURCES mode, fault simulation not yet supported")
    endif
  endif
