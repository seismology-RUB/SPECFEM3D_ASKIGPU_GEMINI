...
! write seismograms to text files
  ...
  use specfem_par, only: myrank,number_receiver_global,NPROC, &
          ...
          nlength_seismogram, SEISMO_PATH
  ...
  if (.not. WRITE_SEISMOGRAMS_BY_MAIN) then
    ...
    if (SU_FORMAT) then
      ...
      call write_output_SU(all_seismograms,nrec_store,istore,SEISMO_PATH)
    else
      ! ASCII output
      if (SAVE_ALL_SEISMOS_IN_ONE_FILE) then
        ...
        if (USE_BINARY_FOR_SEISMOGRAMS) then
          if (seismo_offset == 0) then
            open(unit=IOUT,file=trim(SEISMO_PATH)//trim(sisname)//'.bin',status='unknown', &
                 form='unformatted',action='write')
          else
            open(unit=IOUT,file=trim(SEISMO_PATH)//trim(sisname)//'.bin',status='old',position='append', &
                 form='unformatted',action='write')
          endif
        else
          if (seismo_offset == 0) then
            open(unit=IOUT,file=trim(SEISMO_PATH)//trim(sisname)//'.ascii',status='unknown', &
                 form='formatted',action='write')
          else
            open(unit=IOUT,file=trim(SEISMO_PATH)//trim(sisname)//'.ascii',status='old',position='append', &
                 form='formatted',action='write')
          endif
        endif
      endif
...
!=====================================================================
  ...
  use constants, only: CUSTOM_REAL,NDIM,MAX_STRING_LEN,NB_RUNS_ACOUSTIC_GPU

  use specfem_par, only: DT,t0,it, &
    NSTEP,ASDF_FORMAT,WRITE_SEISMOGRAMS_BY_MAIN, &
    SIMULATION_TYPE,station_name,network_name, &
    nrec,nrec_local,nlength_seismogram,SEISMO_PATH
  ...
  character(len=MAX_STRING_LEN) :: sisname,runname
  ...
  do iorientation = 1,number_of_components
    ...
    ! Add Gemini eventID to seismogram name do avoid overwriting files for different events. TM
    ! Undo that change since very inconvenient when using code together with ASKI. WF
    ! if ((COUPLE_WITH_INJECTION_TECHNIQUE) .and.(INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_GEMINI)) then
    !    sisname = trim(GEMINI_EVENTID) // '.' // trim(sisname)
    ! end if
    ...
    if (ASDF_FORMAT) then
      ...
      call write_output_ASCII_or_binary(one_seismogram, &
                                        ...
                                        iorientation,sisname,SEISMO_PATH)
    endif
...
! write adjoint seismograms (displacement) to text files
  ...
  if (SU_FORMAT) then
    ...
    ! write ONE binary file for all receivers (nrec_local) within one proc
    ! SU format, with 240-byte-header for each trace
    call write_output_SU(all_seismograms,nrec_store,istore,OUTPUT_FILES)
  else
    ! ASCII ouput
    do irec_local = 1,nrec_local
...
!=====================================================================
  ...
  !Parameter added to the Par_File so it can be used. (TM)
  use specfem_par, only: SUPPRESS_IRIS_CONVENTION
