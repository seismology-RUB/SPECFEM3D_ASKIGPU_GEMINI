...
!  use iso_Fortran_env
  ...
  use specfem_par, only: seismo_offset,DT,NSTEP,subsamp_seismos,OUTPUT_FILES,&
                         SEISMO_PATH,WRITE_SEISMOGRAMS_BY_MAIN
  ...
  if (seismo_offset == 0) then
    if (myrank == 0) then
      ! user output
      write(IMAIN,*) 'creating ASDF file: ',trim(SEISMO_PATH) // "synthetic.h5"
      call flush_IMAIN()
      ...
      call ASDF_create_new_file_serial_f(trim(SEISMO_PATH) // "synthetic.h5" // C_NULL_CHAR, file_id)

      ! checks file identifier
      if (file_id == 0) then
        print *,'Error: ASDF failed to create new file',trim(SEISMO_PATH) // "synthetic.h5"
  ...
  if (WRITE_SEISMOGRAMS_BY_MAIN) then
    ...
    if (myrank == 0) then
      ...
      call ASDF_open_serial_f(trim(SEISMO_PATH)//"synthetic.h5" // C_NULL_CHAR, file_id)
      ! checks file identifier
      if (file_id == 0) then
        print *,'Error: ASDF failed to open file',trim(SEISMO_PATH) // "synthetic.h5"
    ...
    call ASDF_open_f(trim(SEISMO_PATH)//"synthetic.h5" // C_NULL_CHAR, comm, file_id)
    ! checks file identifier
    if (file_id == 0) then
      print *,'Error rank ',myrank,': ASDF failed to open file',trim(SEISMO_PATH) // "synthetic.h5"
      call exit_MPI(myrank,'Error opening ASDF file for synthetics')
    endif
