...
!----
  ...
  use specfem_par, only: USE_FORCE_POINT_SOURCE,USE_RICKER_TIME_FUNCTION, &
      ...
      NSOURCES,MULTIPLE_SEQUENTIAL_SOURCES,SEQUENTIAL_SOURCES_MODE,SEISMO_PATH
  ...
  if (USE_FORCE_POINT_SOURCE) then
    ...
    if (myrank == 0) then
      !
      ! if COUPLE_WITH_INJECTION_TECHNIUE is on and SEQUENTIAL_SOURCES_MODE == 3
      ! we need to go into the get_force routine
      ! the case COUPLE_WITH_INJECTION_TECHNIUE is off and SEQUENTIAL_SOURCES_MODE /= 2
      ! is already excluded in read_sequential_sources_description
      !
      if (MULTIPLE_SEQUENTIAL_SOURCES .and. (SEQUENTIAL_SOURCES_MODE == 2)) then
        call get_sequential_force(tshift_src,hdur,lat,long,depth,NSOURCES,&
                    min_tshift_src_original,factor_force_source, comp_dir_vect_source_E,comp_dir_vect_source_N,&
    ...
    if (myrank == 0) then
      !
      ! if COUPLE_WITH_INJECTION_TECHNIUE is on and SEQUENTIAL_SOURCES_MODE == 3
      ! we need to go into the get_cmt routine
      ! the case COUPLE_WITH_INJECTION_TECHNIUE is off and SEQUENTIAL_SOURCES_MODE /= 1
      ! is already excluded in read_sequential_sources_description
      !
      if (MULTIPLE_SEQUENTIAL_SOURCES .and. (SEQUENTIAL_SOURCES_MODE == 1)) then
        call get_sequential_cmt(tshift_src,hdur,lat,long,depth,moment_tensor, &
  ...
  !
  !  if SEQUENTIAL_SOURCES_MODE == 3 this broadcast is already done
  !  if SEQUENTIAL_SOURCES_MODE /= 3 it is required
  !
  if (MULTIPLE_SEQUENTIAL_SOURCES .and. (SEQUENTIAL_SOURCES_MODE /= 3)) then
    call bcast_all_string(ASKI_output_ID)
    call bcast_all_singlel(ASKI_MAIN_FILE_WRITE)
    call bcast_all_string(ASKI_outfile)
  ...
  if (myrank == 0) then
    ...
    if (COUPLE_WITH_INJECTION_TECHNIQUE) then
      ...
      write(IMAIN,*) '  source (inside the mesh) will be ignored if we are coupling with DSM/AxiSEM/FK/GEMINI,'
