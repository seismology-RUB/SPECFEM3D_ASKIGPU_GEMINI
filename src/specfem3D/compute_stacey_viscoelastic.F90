...
! absorbing boundary term for elastic media (Stacey conditions)
  ...
  ! added by WF for GEMINI coupling
  ...
  use specfem_par, only: it_dsm, it_fk, Veloc_dsm_boundary, Tract_dsm_boundary, Veloc_axisem, Tract_axisem, Tract_axisem_time, &
                         GEMINI_HDF_FILE_ID,HDF_XFERPRP,GEMINI_DT,GEMINI_TLEN,deltat, veltrac1 ,veltrac2, &
                         DT, NSTEP
...
! *********************************************************************************
  ...
  integer(kind=8), dimension(2) :: dimsslab
  integer :: kgemini
  integer, save :: itgem

  double precision :: tspec,alip,blip
  double precision, save :: tgemb
  ...
  ! This line is moved here to ensure the coupling with GEMINI. Processes that do not contain boundary points do not appear
  ! in the files that supply the injection seismograms. This results in an error that terminates the program as the path
  ! to the seismograms is not present in the hdf file. TM TM
  ...
  if (COUPLE_WITH_INJECTION_TECHNIQUE) then
    ...
    else if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_GEMINI) then
      if (iphase == 1) then
        write(crank,'(i6.6)') myrank
        dimsslab = [6,num_abs_boundary_faces*NGLLSQUARE]
        count = [6,num_abs_boundary_faces*NGLLSQUARE,1]
        if (it == 1) then                                  ! read 2 samples from Gemini synthetics for interpolation
           itgem = 2
           tgemb = GEMINI_DT; tspec = 0.d0
           offset = [0,0,0]
           call readArrayHDFWrapper(GEMINI_HDF_FILE_ID,"/proc_"//crank//"/velocityTraction",arra,&
                errmsg,xferprp = HDF_XFERPRP,dimsslab = dimsslab,offset = offset,count = count)
           if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read velocity-traction data")
           veltrac1 => arra%get2d(); call arra%deassoc()
           offset = [0,0,1]
           call readArrayHDFWrapper(GEMINI_HDF_FILE_ID,"/proc_"//crank//"/velocityTraction",arra,&
                errmsg,xferprp = HDF_XFERPRP,dimsslab = dimsslab,offset = offset,count = count)
           if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read velocity-traction data")
           veltrac2 => arra%get2d(); call arra%deassoc()
           alip = 1.d0; blip = 0.d0                       ! linear interpolation constants
        else
           tspec = (it-1)*deltat
           if (tspec > tgemb) then
              itgem = itgem+1;
              tgemb = (itgem-1)*GEMINI_DT
              if (tgemb > GEMINI_TLEN) then
                 print*, "Gemini seismogram length: ", GEMINI_TLEN
                 print*, "Specfem seismogram length: ", NSTEP*DT
                 print*, "Suggested number of time steps: ", GEMINI_TLEN*GEMINI_DT/DT
                 call exit_MPI_without_rank("End of Gemini synthetics reached")
              end if
              offset = [0,0,itgem-1]
              call readArrayHDFWrapper(GEMINI_HDF_FILE_ID,"/proc_"//crank//"/velocityTraction",arra,&
                   errmsg,xferprp = HDF_XFERPRP,dimsslab = dimsslab,offset = offset,count = count)
              if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read velocity-traction data")
              deallocate(veltrac1)   ! dellocate veltrac1-storage here. It is the last pointer to the wavefield of the previous timestep
              veltrac1 => veltrac2
              veltrac2 => arra%get2d(); call arra%deassoc()
           endif
           alip = (tgemb-tspec)/GEMINI_DT; blip = 1.d0-alip       ! linear interpolation constants
        endif
      endif
    ...
    if (ispec_is_elastic(ispec)) then
      ...
      do igll = 1,NGLLSQUARE
        ...
        if (COUPLE_WITH_INJECTION_TECHNIQUE) then
          ...
          else if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_GEMINI) then !! WF add GEMINI
            kgemini = igll + NGLLSQUARE*(iface - 1)

            vx = vx - (alip*veltrac1(1,kgemini)+blip*veltrac2(1,kgemini))
            vy = vy - (alip*veltrac1(2,kgemini)+blip*veltrac2(2,kgemini))
            vz = vz - (alip*veltrac1(3,kgemini)+blip*veltrac2(3,kgemini))
        ...
        if (COUPLE_WITH_INJECTION_TECHNIQUE) then
          if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_DSM) then    !! To verify for NOBU version
            ...
            tx = tx - (alip*veltrac1(4,kgemini)+blip*veltrac2(4,kgemini))
            ty = ty - (alip*veltrac1(5,kgemini)+blip*veltrac2(5,kgemini))
            tz = tz - (alip*veltrac1(6,kgemini)+blip*veltrac2(6,kgemini))
          else if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_FK) then
            ! added by Ping Tong (TP / Tong Ping) for the FK3D calculation
            tx = tx - tx_FK(ipt)
            ty = ty - ty_FK(ipt)
...
! absorbing boundary term for elastic media (Stacey conditions)
  ...
  ! added by WF and TM for GEMINI coupling
  use anyRankRealArray
  use hdfWrapper
  use errorMessage

  use shared_parameters, only: COUPLE_WITH_INJECTION_TECHNIQUE,INJECTION_TECHNIQUE_TYPE,UNDO_ATTENUATION_AND_OR_PML

  ! Added by TM for coupling with Gemini
  use specfem_par, only: GEMINI_HDF_FILE_ID,HDF_XFERPRP,GEMINI_DT,GEMINI_TLEN,deltat, &
                         veltrac1 ,veltrac2, DT, veltrac1_cuda, veltrac2_cuda!, veltrac_all
...
! real(kind=CUSTOM_REAL) :: Tract_dsm_boundary(3,Ntime_step_dsm,NGLLSQUARE,num_abs_boundary_faces)
  ...
  !integer :: kgemini
  integer, save :: itgem
  real(kind=CUSTOM_REAL) :: tspec,alip,blip
  real(kind=CUSTOM_REAL), save :: tgemb
  character(len=6) :: crank
  ...
  if (COUPLE_WITH_INJECTION_TECHNIQUE) then
    if (old_DSM_coupling_from_Vadim) then
      if (iphase == 1) then
        if (mod(it_dsm,Ntime_step_dsm+1) == 0 .or. it == 1) then
          call exit_MPI_without_rank('DK DK old_DSM_coupling_from_Vadim support is discontinued')
!         call read_dsm_file(Veloc_dsm_boundary,Tract_dsm_boundary,num_abs_boundary_faces,it_dsm)
        endif
      endif
    else if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_GEMINI) then
      if (iphase == 1) then
        ...
        if (it == 1) then                                  ! read 2 samples from Gemini synthetics for interpolation
           itgem = 2
           tgemb = GEMINI_DT; tspec = 0.0_CUSTOM_REAL
           offset = [0,0,0]
           call readArrayHDFWrapper(GEMINI_HDF_FILE_ID,"/proc_"//crank//"/velocityTraction",arra,&
                errmsg,xferprp = HDF_XFERPRP,dimsslab = dimsslab,offset = offset,count = count)
           if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read velocity-traction data")
           veltrac1 => arra%get2d(); call arra%deassoc()
!           veltrac1 => veltrac_all(:,:,1)
           offset = [0,0,1]
           call readArrayHDFWrapper(GEMINI_HDF_FILE_ID,"/proc_"//crank//"/velocityTraction",arra,&
                errmsg,xferprp = HDF_XFERPRP,dimsslab = dimsslab,offset = offset,count = count)
           if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read velocity-traction data")
           veltrac2 => arra%get2d(); call arra%deassoc()
!           veltrac2 => veltrac_all(:,:,2)
           alip = 1.d0; blip = 0.d0                       ! linear interpolation constants
           allocate(veltrac1_cuda(6, size(veltrac1(1,:))), veltrac2_cuda(6, size(veltrac1(1,:))))

           ! Allocate memory for the velocity-traction arrays on the GPU
           call PREPARE_VELTRAC_GPU(Mesh_pointer, 6*size(veltrac1(1,:)))

           ! Copy veltrac to GPU
           veltrac1_cuda = veltrac1
           veltrac2_cuda = veltrac2
           call VELTRAC_TO_GPU(Mesh_pointer, veltrac1_cuda, veltrac2_cuda, 6*size(veltrac1(1,:)))
        else
           tspec = (it-1)*deltat
           if (tspec > tgemb) then
              itgem = itgem+1;
...
!              veltrac2 => veltrac_all(:,:,itgem)
           ...
           alip = (tgemb-tspec)/GEMINI_DT; blip = 1.0_CUSTOM_REAL - alip       ! linear interpolation constants
        endif
        call INTERPOLATIONPARAMETER_TO_GPU(Mesh_pointer, alip, blip)
      endif
    else
      !! MODIFS DE NOBU 2D
    endif
  ...
  !if (it == 10) call exit_MPI_without_rank("ten time steps done")
  !! CD CD : begin
  !! For coupling with DSM
  if (COUPLE_WITH_INJECTION_TECHNIQUE) then !! To verify for NOBU version
    if (iphase == 1) then
