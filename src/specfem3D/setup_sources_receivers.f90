...
!------------------------------------------------------------------------------------------
!
  subroutine allocate_arrays_for_sources()
  ...
  end subroutine allocate_arrays_for_sources
!
!-------------------------------------------------------------------------------------------------
!
...
!
  ...
  ! utm_x_source and utm_y_source not yet allocated in MULTIPLE_SEQUENTIAL_SOURCES case
  !
  if (allocated(utm_x_source)) then
...
!
  ...
  ! forward simulations
  if (SIMULATION_TYPE == 1 .or. SIMULATION_TYPE == 3) then
    allocate(sourcearray(NDIM,NGLLX,NGLLY,NGLLZ),stat=ier)                     ! local use only (WF), can be removed from specfem_par
    ...
    enddo
    deallocate(sourcearray)                    ! only used in previous loop
  else
    ! SIMULATION_TYPE == 2
    ! allocate dummy array (needed for subroutine calls)
    allocate(sourcearrays(1,1,1,1,1),stat=ier)
...
!
  ...
  integer :: i,j,k,ispec,nlength_seismogram_prl
  ...
  ! preliminary seismogram array length just for this routine
  ! real value needed first in allocate_receivers_seismograms
  nlength_seismogram_prl = NTSTEP_BETWEEN_OUTPUT_SEISMOS/subsamp_seismos
  ...
  if (myrank == 0) then
    ...
    size_s = dble(maxrec) * dble(NDIM) * dble(nlength_seismogram_prl * CUSTOM_REAL / 1024. / 1024. )
    sizeval = 0.d0
...
!-------------------------------------------------------------------------------------------------
!
  subroutine setup_receivers_only()

  use specfem_par
  implicit none
  integer :: nstepmax

  ! preliminary NSTEP and NTSTEP_BETWEEN_OUTPUT_SEISMOS
  ! is changed later when getting injection sources
  ! use a maximum NSTEP for multiple injection else use NSTEP from Par_file
  if (MULTIPLE_SEQUENTIAL_SOURCES .and. (SEQUENTIAL_SOURCES_MODE == 3)) then
    call get_max_length_seismograms(nstepmax)
    NSTEP = nstepmax
    NTSTEP_BETWEEN_OUTPUT_SEISMOS = NSTEP
    if (myrank == 0) then
      write(IMAIN,*)
      write(IMAIN,*) "Max NSTEP used with injection sources: ",NSTEP
      write(IMAIN,*)
      call flush_IMAIN()
    endif
  endif
  ...
  end subroutine setup_receivers_only
!
!-------------------------------------------------------------------------------------------------
!
  subroutine setup_sources_only()
  ...
  ! write source VTK files for Paraview
  call setup_sources_VTKfile()

  ! synchronizes processes
  call synchronize_all()

  end subroutine setup_sources_only
!
!-------------------------------------------------------------------------------------------------
!
  subroutine setup_receivers_VTKfile()

  use constants, only: CUSTOM_REAL,MAX_STRING_LEN,OUTPUT_FILES,IOUT_VTK,myrank

  use specfem_par, only: nrec,NGNOD,NSPEC_AB,NGLOB_AB, &
    xstore,ystore,zstore,ibool, &
    ispec_selected_rec,islice_selected_rec,xi_receiver,eta_receiver,gamma_receiver
  ...
  double precision :: xmesh,ymesh,zmesh
  real(kind=CUSTOM_REAL),dimension(NGNOD) :: xelm,yelm,zelm
  integer :: ia,ispec,irec,ier,totalpoints
  ...
  if (myrank == 0) then
    ! vtk file
    open(IOUT_VTK,file=trim(OUTPUT_FILES)//'/receivers.vtk',status='unknown',iostat=ier)
    if (ier /= 0) stop 'error opening receivers.vtk file'
    ! vtk header
    write(IOUT_VTK,'(a)') '# vtk DataFile Version 2.0'
    write(IOUT_VTK,'(a)') 'Receiver VTK file'
    write(IOUT_VTK,'(a)') 'ASCII'
    write(IOUT_VTK,'(a)') 'DATASET POLYDATA'
    write(IOUT_VTK, '(a,i6,a)') 'POINTS ', totalpoints, ' float'
  endif
  ...
  end subroutine setup_receivers_VTKfile
  !
  !-------------------------------------------------------------------
  !
    subroutine setup_sources_VTKfile()

  use constants, only: CUSTOM_REAL,MAX_STRING_LEN,OUTPUT_FILES,IOUT_VTK,myrank

  use specfem_par, only: SIMULATION_TYPE,NSOURCES,NGNOD,NSPEC_AB,NGLOB_AB, &
    xstore,ystore,zstore,ibool, &
    ispec_selected_source,islice_selected_source,xi_source,eta_source,gamma_source
  ...
  double precision :: xmesh,ymesh,zmesh
  real(kind=CUSTOM_REAL),dimension(NGNOD) :: xelm,yelm,zelm
  integer :: ia,ispec,isource,ier,totalpoints

  ! determines number of points for vtk file
  totalpoints = NSOURCES

  if (myrank == 0) then
    ! vtk file
    open(IOUT_VTK,file=trim(OUTPUT_FILES)//'/sources.vtk',status='unknown',iostat=ier)
    if (ier /= 0) stop 'error opening sources.vtk file'
    ! vtk header
    write(IOUT_VTK,'(a)') '# vtk DataFile Version 2.0'
    write(IOUT_VTK,'(a)') 'Source VTK file'
    write(IOUT_VTK,'(a)') 'ASCII'
    write(IOUT_VTK,'(a)') 'DATASET POLYDATA'
    write(IOUT_VTK, '(a,i6,a)') 'POINTS ', totalpoints, ' float'
  endif
  ...
  end subroutine setup_sources_VTKfile
!
!-------------------------------------------------------------------------------------
!
  subroutine allocate_receivers_seismograms()
  ...
  ! set nlength_seismogram here to allow dynamic changes with multiple sequential sources
  ...
  end subroutine allocate_receivers_seismograms
!
!--------------------------------------------------------------------------
!
  subroutine zero_receivers_seismograms()
  ...
  end subroutine zero_receivers_seismograms
