...
!=====================================================================
  ...
  use hdfWrapper                     ! added by WF
  use errorMessage                   ! added by WF
  use specfem_par
...
#endif
  type (error_message) :: errmsg     ! added by WF
...
!
  ...
  if (myrank == 0) then
    write(IMAIN,*)
    write(IMAIN,*) 'Starting time iteration loop after running for ',wtime()-time_specfem_init,' seconds'
    write(IMAIN,*)
    call flush_IMAIN()
  endif
...
!           if (height >= 3000.d0) then
  ...
  ! evaluate displacements and strains for ASKI output on GLL points (WF)
  ...
  ! Get ASKI spectra from GPU for output
  if (GPU_MODE) call it_transfer_ASKI_from_GPU()
  ...
  ! free seismograms and ASKI efactors
  if (GPU_MODE) then
    call free_efactors_ASKI_GPU()
    call free_seismograms_GPU()
  end if

  ! Deallocate GEMINI Arrays (TM)
  if (COUPLE_WITH_INJECTION_TECHNIQUE) then
    if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_GEMINI) then
      ! free space allocated by readArrayHdfWrapper
      if (associated(veltrac1)) deallocate(veltrac1)
      if (associated(veltrac2)) deallocate(veltrac2)
!      if (associated(veltrac_all)) deallocate(veltrac_all)
      if (GPU_MODE) then
        if (allocated(veltrac1_cuda)) deallocate(veltrac1_cuda)
        if (allocated(veltrac2_cuda)) deallocate(veltrac2_cuda)
      end if
    end if
  end if

  !! WF for Gemini injection, close HDF stuff
  if (COUPLE_WITH_INJECTION_TECHNIQUE) then
    if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_GEMINI) then
      call closeFileHDFWrapper(GEMINI_HDF_FILE_ID,errmsg)
      if (.level.errmsg == 2) call exit_MPI_without_rank('error closing HDF file')
    endif
  endif
  ...
  if (myrank == 0) then
    write(IMAIN,*)
    write(IMAIN,*) 'End of iterate_time after running for ',wtime()-time_specfem_init,' seconds'
    write(IMAIN,*)
    call flush_IMAIN()
  endif
...
!
  ...
  call prepare_cleanup_device(Mesh_pointer,ACOUSTIC_SIMULATION,ELASTIC_SIMULATION, &
                              ...
                              APPROXIMATE_HESS_KL, COUPLE_WITH_INJECTION_TECHNIQUE, INJECTION_TECHNIQUE_TYPE)

  end subroutine it_cleanup_GPU
!
!-------------------------------------------------------------------------------------------------
!
  subroutine free_seismograms_GPU()
  ...
  call free_seismograms_device(Mesh_pointer)

  end subroutine free_seismograms_GPU
