...
! for coupling with EXTERNAL CODE !! CD CD modify here
  if (COUPLE_WITH_INJECTION_TECHNIQUE .or. SAVE_RUN_BOUN_FOR_KH_INTEGRAL) then
    if (myrank == 0) then
      ...
      case (INJECTION_TECHNIQUE_IS_GEMINI)
        write(IMAIN,*) ' type of injection technique is GEMINI'
      case default
        stop 'Invalid INJECTION_TECHNIQUE_TYPE chosen, must be 1 == DSM, 2 == AXISEM, 3 == FK or 4 == GEMINI'
      end select
      write(IMAIN,*) '**********************************************'
      write(IMAIN,*) 'Start of couple_with_injection after ',wtime()-time_specfem_init,' seconds.'
      write(IMAIN,*)
    endif
  ...
  if (COUPLE_WITH_INJECTION_TECHNIQUE) then
    ...
    if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_DSM) then
      ...
      call couple_with_injection_prepare_gemini()
    endif
    if (myrank == 0) then
       write(IMAIN,*)
       write(IMAIN,*) 'End of couple_with_injection after ',wtime()-time_specfem_init,' seconds.'
    endif
  endif
...
!
!------------------------------------------------------------------------------------------------
!  prepare coupling to Gemini, open HDF environment and synseis file
!
  subroutine couple_with_injection_prepare_gemini()
    use specfem_par
    use hdfWrapper
    use errorMessage
    implicit none
  ...
  !
    if (MULTIPLE_SEQUENTIAL_SOURCES .and. SEQUENTIAL_SOURCES_MODE == 3) then
       call get_sequential_injection()
    endif
  !
    filename = trim(TRACTION_PATH)//trim(GEMINI_SYNSEIS_FILE)
    call openFileParallelAccessHDFWrapper(filename,GEMINI_HDF_FILE_ID,errmsg)
    if (.level.errmsg == 2) call exit_MPI_without_rank("Could not open HDF file: "//trim(filename))
  !
    call readArrayAttributeHDFWrapper(GEMINI_HDF_FILE_ID,"samplingIntervalLength",arra,errmsg)
    if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read attribute sampleIntervalLength")
    d => arra%get1d(); call arra%deassoc()
    GEMINI_DT = real(d(1),kind=CUSTOM_REAL); GEMINI_TLEN = real(d(2),kind=CUSTOM_REAL)
    deltat = real(DT,kind=CUSTOM_REAL)
   !
   !  Timing information for injection seismograms
   !  twinlen = d(1); tsbuf = d(2); ttmin = d(3); tred = d(4); tend = d(5); tscatlen = d(6)
   !
    call readArrayAttributeHDFWrapper(GEMINI_HDF_FILE_ID,"timing",arra,errmsg)
    if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read attribute timing")
    d => arra%get1d(); call arra%deassoc()
    GEMINI_INJECTION_TIMING = real(d,kind=CUSTOM_REAL)
    deallocate(d)
   !
   !  get event location from event group in injection seismogram HDF
   !  slat, slon in degrees and sdepth in meter
   !
    call openGroupHDFWrapper(GEMINI_HDF_FILE_ID,'Event',evgrid,errmsg)
    if (.level.errmsg == 2) call exit_MPI_without_rank("Could not open event group")
    call readArrayAttributeHDFWrapper(evgrid,'Coordinates',arra,errmsg)
    if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read event coordinates")
    d => arra%get1d()
    GEMINI_INJECTION_EVLOC = real(d,kind=CUSTOM_REAL)
    deallocate(d)
    call  closeGroupHDFWrapper(evgrid,errmsg)
    if (.level.errmsg == 2) call exit_MPI_without_rank("Could not open event group")
   !
    if (NSTEP*deltat > GEMINI_TLEN) then
        print*, NSTEP, deltat, GEMINI_TLEN
        call exit_MPI_without_rank("Specfem simulation length exceeds Gemini seismogram length!")
    end if
   !
   !  read all time samples of injection seimsmograms for given rank
   !
!    if (.false.) then
!    write(crank,'(i6.6)') myrank
!    call readArrayHDFWrapper(GEMINI_HDF_FILE_ID,"/proc_"//crank//"/velocityTraction",arra,&
!                      errmsg,xferprp = HDF_XFERPRP)
!    if (.level.errmsg == 2) call exit_MPI_without_rank("Could not read velocity-traction data")
!    veltrac_all => arra%get3d()
!    call arra%deassoc()
!    call closeFileHDFWrapper(GEMINI_HDF_FILE_ID,errmsg)
!    if (.level.errmsg == 2) call exit_MPI_without_rank('error closing injection HDF file')
!    end if
  !
  end subroutine couple_with_injection_prepare_gemini
