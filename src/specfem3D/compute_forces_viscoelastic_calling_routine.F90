...
! elastic solver
  ...
  !use constants, only INJECTION_TECHNIQUE_IS_GEMINI
  ...
  if (FAULT_SIMULATION) then
    ...
    if (FAULT_SYNCHRONIZE_DISPL_VELOC .and. SIMULATION_TYPE_DYN .and. NPROC > 1) then
      ...
      call synchronizeVelocityDisplacement()
    endif
  endif

  ! Even with the added synchronisation the simualtions still explode in GPU mode.
  !if (COUPLE_WITH_INJECTION_TECHNIQUE .and. NPROC > 1) then
...
! elastic solver
  ...
  subroutine synchronizeVelocityDisplacement()
