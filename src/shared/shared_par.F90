...
! holds input parameters given in DATA/Par_file
  ...
  logical :: SUPPRESS_UTM_PROJECTION    ! WF: choose .true. if spherical mapping is on
  logical :: MAP_TO_SPHERICAL_CHUNK     ! WF: choose .true. for spherical mapping
  ...
  ! seismograms
  character(len=MAX_STRING_LEN) :: SEISMO_PATH
  ...
  logical :: SUPPRESS_IRIS_CONVENTION
  ...
  character(len=MAX_STRING_LEN) :: GEMINI_SYNSEIS_FILE
  logical :: MULTIPLE_SEQUENTIAL_SOURCES
  character(len=MAX_STRING_LEN) :: SEQUENTIAL_SOURCES_DESCRIPTION_FILE
