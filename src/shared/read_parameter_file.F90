...
!=====================================================================
  ...
  use constants, only: &
    myrank, &
    INJECTION_TECHNIQUE_IS_AXISEM,INJECTION_TECHNIQUE_IS_DSM,INJECTION_TECHNIQUE_IS_FK,INJECTION_TECHNIQUE_IS_GEMINI
  ...
  if (myrank == 0) then
    ...
    call read_value_logical(MAP_TO_SPHERICAL_CHUNK, 'MAP_TO_SPHERICAL_CHUNK', ier)   !WF
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'MAP_TO_SPHERICAL_CHUNK         = .true.'
      write(*,*)
    endif
    ...
    call read_value_string(SEISMO_PATH, 'SEISMO_PATH', ier)
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'SEISMO_PATH                 = ./OUTPUT_FILES/'
      write(*,*)
    endif
    ...
    call read_value_logical(SUPPRESS_IRIS_CONVENTION, 'SUPPRESS_IRIS_CONVENTION', ier)
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'SUPPRESS_IRIS_CONVENTION = .false.'
      write(*,*)
    endif
    ...
    if (COUPLE_WITH_INJECTION_TECHNIQUE) then
      if (INJECTION_TECHNIQUE_TYPE /= INJECTION_TECHNIQUE_IS_DSM .and. &
         ...
         INJECTION_TECHNIQUE_TYPE /= INJECTION_TECHNIQUE_IS_GEMINI .and. &
    ...
    !-------------------------------------------------------
    !  Read parameters for coupling with GEMINI
    !
    call read_value_string(GEMINI_SYNSEIS_FILE, 'GEMINI_SYNSEIS_FILE', ier)
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'GEMINI_SYNSEIS_FILE             = specfem_coupling_synseis.hdf'
      write(*,*)
    endif
    !--------------------------------------------------------
    !  for multiple sequential runs with different sources
    call read_value_logical(MULTIPLE_SEQUENTIAL_SOURCES, 'MULTIPLE_SEQUENTIAL_SOURCES', ier)
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'MULTIPLE_SEQUENTIAL_SOURCES            = .false.'
      write(*,*)
    endif
    call read_value_string(SEQUENTIAL_SOURCES_DESCRIPTION_FILE, 'SEQUENTIAL_SOURCES_DESCRIPTION_FILE', ier)
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'SEQUENTIAL_SOURCES_DESCRIPTION_FILE             = kernel_disp_specfem_injection'
      write(*,*)
    endif
...
! broadcasts parameters to all processes
  ...
  call bcast_all_singlel(MAP_TO_SPHERICAL_CHUNK)
  ...
  ! seismograms
  call bcast_all_string(SEISMO_PATH)
  call bcast_all_singlei(NTSTEP_BETWEEN_OUTPUT_SEISMOS)
  call bcast_all_singlel(SUPPRESS_IRIS_CONVENTION)
  ...
  call bcast_all_string(GEMINI_SYNSEIS_FILE)

  ! many sequential runs
  call bcast_all_singlel(MULTIPLE_SEQUENTIAL_SOURCES)
  call bcast_all_string(SEQUENTIAL_SOURCES_DESCRIPTION_FILE)
