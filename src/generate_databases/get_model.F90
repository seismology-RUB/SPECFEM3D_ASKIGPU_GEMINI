...
! DP: not sure if this here should check if (COUPLE_WITH_INJECTION_TECHNIQUE .or. MESH_A_CHUNK_OF_THE_EARTH) ..
  if (COUPLE_WITH_INJECTION_TECHNIQUE) then
    ...
    if (myrank == 0) then
      ...
      case (INJECTION_TECHNIQUE_IS_GEMINI)
        write(IMAIN,*) '     INJECTION TECHNIQUE TYPE = ', INJECTION_TECHNIQUE_TYPE,' (GEMINI) '
      case default
        stop 'Invalid INJECTION_TECHNIQUE_TYPE chosen, must be 1 == DSM, 2 == AXISEM or 3 == FK or 4 == GEMINI'
      end select
      write(IMAIN,*)
      write(IMAIN,*)
    endif
    ...
    if (INJECTION_TECHNIQUE_TYPE /= INJECTION_TECHNIQUE_IS_FK &
        .and. INJECTION_TECHNIQUE_TYPE /= INJECTION_TECHNIQUE_IS_GEMINI) then
  ...
  do ispec = 1, nspec
    ...
    ! get element center coordinates, assume odd number of GLL points
    ! pass to get_model_values
    iglob = ibool((NGLLX+1)/2,(NGLLY+1)/2,(NGLLZ+1)/2,ispec)
    xc = xstore_unique(iglob)
    yc = ystore_unique(iglob)
    zc = zstore_unique(iglob)
      ...
      do j = 1, NGLLY
        do i = 1, NGLLX
          ...
          call get_model_values(mat_prop,nmat_ext_mesh, &
                                ...
                                xmesh,ymesh,zmesh,xc,yc,zc,ispec, &
...
!
  ...
  use generate_databases_par, only: IMODEL,IMODEL_DEFAULT,IMODEL_GLL,IMODEL_1D_PREM,IMODEL_1D_CASCADIA,IMODEL_1D_SOCAL, &
    ...
    IMODEL_1D_PREM_PB,IMODEL_GLL,IMODEL_SEP,IMODEL_COUPLED,IMODEL_ASKI, &
  ...
  double precision, intent(in) :: xmesh,ymesh,zmesh,xc,yc,zc
...
! *********************************************************************************
  ...
  case (IMODEL_ASKI)
    ! gets model values for ASKI
    ! material values determined by mesh properties (in case no background model is chosen)
    call model_default(mat_prop,nmat_ext_mesh, &
                       undef_mat_prop,nundefMat_ext_mesh, &
                       imaterial_id,imaterial_def, &
                       xmesh,ymesh,zmesh, &
                       ...
                       phi,tort,kxx,kxy,kxz,kyy,kyz,kzz, &
                       c11,c12,c13,c14,c15,c16,c22,c23,c24,c25,c26,c33,c34,c35,c36,c44,c45,c46,c55,c56,c66)
    call model_aski_values(xmesh,ymesh,zmesh,xc,yc,zc,ispec,rho,vp,vs,qkappa_atten,qmu_atten,iflag_aniso,idomain_id)
