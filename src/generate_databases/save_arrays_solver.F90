...
!
  ...
  if (INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_GEMINI) then
     filename = prname(1:len_trim(prname))//'absorbing_boundaries_for_gemini.txt'
     open(IOUT,file=filename(1:len_trim(filename)),status='unknown',iostat=ier)
     write(IOUT,'(i10)') nspec2d_xmin+nspec2D_xmax+nspec2D_ymin+nspec2D_ymax
     write(IOUT,'(i10)') NSPEC2D_BOTTOM
     write(IOUT,'(i10)') NSPEC2D_TOP
  else
