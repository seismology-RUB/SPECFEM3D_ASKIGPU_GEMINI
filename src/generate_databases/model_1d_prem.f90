...
! given a GLL point, returns super-imposed velocity model values
  ...
  use shared_parameters, only: MAP_TO_SPHERICAL_CHUNK    ! WF
  ...
  ! Parameter needed for the use of axesRotation
  double precision :: xlc, ylc, zlc, delta, xi
  ...
  !RMOHO = 6346600.d0
  RMOHO = 6346000.d0 !Modified to conform with PREM used in GEMINI
  R80  = 6291000.d0
  R220 = 6151000.d0
  R400 = 5971000.d0
  R600 = 5771000.d0
  ...
  RCMB = 3480000.d0
  !RICB = 1221000.d0
  RICB = 1221500.d0 !Modified to conform with PREM used in GEMINI

  if (MAP_TO_SPHERICAL_CHUNK) then
    ! calculate the true radius of a point
    call coordinatesLCfromRCAxesRotation(mc_pid/2.d0, dble(xloc), dble(yloc), R_EARTH + dble(zloc), xlc, ylc, zlc)
