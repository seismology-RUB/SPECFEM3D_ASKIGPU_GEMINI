...
!
  ...
  ! save gll point cooridinate to local variable as they need to be modified when a spherical chunk is considered.
  xloc = xmesh
  yloc = ymesh
  zloc = zmesh

  if (MAP_TO_SPHERICAL_CHUNK) then
    call mapSphericalChunkToCartesianBox(xloc,yloc,zloc)
  end if
  ! determine spacing and cell for linear interpolation
  spac_x = (xloc - ORIG_X(imat)) / SPACING_X(imat)
  spac_y = (yloc - ORIG_Y(imat)) / SPACING_Y(imat)
  spac_z = (zloc - ORIG_Z(imat)) / SPACING_Z(imat)
  ...
  if (z_tomography(imat,p4+1) == z_tomography(imat,p0+1)) then
     ...
     gamma_interp_z1 = (zloc-z_tomography(imat,p0+1))/(z_tomography(imat,p4+1)-z_tomography(imat,p0+1))
  endif
  ...
  if (z_tomography(imat,p5+1) == z_tomography(imat,p1+1)) then
     ...
     gamma_interp_z2 = (zloc-z_tomography(imat,p1+1))/(z_tomography(imat,p5+1)-z_tomography(imat,p1+1))
  endif
  ...
  if (z_tomography(imat,p6+1) == z_tomography(imat,p2+1)) then
     ...
     gamma_interp_z3 = (zloc-z_tomography(imat,p2+1))/(z_tomography(imat,p6+1)-z_tomography(imat,p2+1))
  endif
  ...
  if (z_tomography(imat,p7+1) == z_tomography(imat,p3+1)) then
     ...
     gamma_interp_z4 = (zloc-z_tomography(imat,p3+1))/(z_tomography(imat,p7+1)-z_tomography(imat,p3+1))
  endif
