...
extern EXTERN_LANG
void FC_FUNC_(prepare_constants_device_aski,
              PREPARE_CONSTANTS_DEVICE_ASKI)(long* Mesh_pointer,
                                        int* nstep,
                                        int* nf,
                                        int* ntl,
                                        int* np_local,
  ...
  TRACE("prepare_constants_device_aski");

  Mesh* mp = (Mesh*)(*Mesh_pointer);
  mp->ASKI_nf = *nf;
  mp->ASKI_np_local = *np_local;
  mp->ASKI_ntl = *ntl;
  mp->ASKI_store_veloc = *store_veloc;
  gpuCopy_todevice_int((void**)&mp->d_ASKI_indx_local,indx_local,*np_local*4);
  gpuCopy_todevice_realw((void**)&mp->d_ASKI_local_disp,disp,3*(*np_local));
  gpuCopy_todevice_realw((void**)&mp->d_ASKI_local_strain,strain,6*(*np_local));
  gpuCopy_todevice_realw((void**)&mp->d_ASKI_efactors_re,efactors_re,(*nf)*(*nstep));
  gpuCopy_todevice_realw((void**)&mp->d_ASKI_efactors_im,efactors_im,(*nf)*(*nstep));
  gpuCopy_todevice_realw((void**)&mp->d_ASKI_local_spectra_re,specre,9*(*nf)*(*np_local));
  gpuCopy_todevice_realw((void**)&mp->d_ASKI_local_spectra_im,specim,9*(*nf)*(*np_local));
  gpuCopy_todevice_int((void**)&mp->d_ASKI_ntaper_start,ntapstart,*np_local);
  gpuCopy_todevice_realw((void**)&mp->d_ASKI_taper_values,tapval,*ntl);

  GPU_ERROR_CHECKING("prepare_constants_device_aski");
}
...
extern EXTERN_LANG
void FC_FUNC_(prepare_cleanup_device_aski,
              PREPARE_CLEANUP_DEVICE_ASKI)(long* Mesh_pointer) {

  TRACE("prepare_cleanup_device_aski");
  ...
  // frees memory on GPU
  // mesh
  gpuFree(mp->d_ASKI_indx_local);
  gpuFree(mp->d_ASKI_local_disp);
  gpuFree(mp->d_ASKI_local_strain);
  gpuFree(mp->d_ASKI_efactors_re);
  gpuFree(mp->d_ASKI_efactors_im);
  gpuFree(mp->d_ASKI_local_spectra_re);
  gpuFree(mp->d_ASKI_local_spectra_im);
  gpuFree(mp->d_ASKI_ntaper_start);
  gpuFree(mp->d_ASKI_taper_values);
}

extern EXTERN_LANG
void FC_FUNC_(free_efactors_device_aski,
              FREE_EFACTORS_DEVICE_ASKI)(long* Mesh_pointer) {

  TRACE("free_efactors_device_aski");
  Mesh* mp = (Mesh*)(*Mesh_pointer);
  gpuFree(mp->d_ASKI_efactors_re);
  gpuFree(mp->d_ASKI_efactors_im);
}
...
extern EXTERN_LANG
void FC_FUNC_(free_seismograms_device,
              FREE_SEISMOGRAMS_DEVICE)(long* Mesh_pointer) {

  TRACE("free_seismograms_device");
  Mesh* mp = (Mesh*)(*Mesh_pointer);
  if (mp->nrec_local > 0){
