...
extern EXTERN_LANG
void FC_FUNC_(transfer_aski_spectra_from_device,
              TRANSFER_ASKI_SPECTRA_FROM_DEVICE)(int* size, realw* specre, realw* specim, long* Mesh_pointer) {

  TRACE("transfer_aski_spectra_from_device");

  Mesh* mp = (Mesh*)(*Mesh_pointer); //get mesh pointer out of fortran integer container

  gpuMemcpy_tohost_realw(specre,mp->d_ASKI_local_spectra_re,*size);
  gpuMemcpy_tohost_realw(specim,mp->d_ASKI_local_spectra_im,*size);
}
