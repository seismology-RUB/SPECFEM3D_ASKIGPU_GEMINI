...
/* Coupling with injection technique - GEMINI

/* ----------------------------------------------------------------------------------------------- */

extern EXTERN_LANG
void FC_FUNC_(veltrac_to_gpu,
              veltrac_to_gpu)(long* Mesh_pointer,
                              realw* veltrac1,
                              realw* veltrac2,
                              int* size) {


    TRACE("veltrac_to_gpu");
    Mesh* mp = (Mesh*)(*Mesh_pointer); //get mesh pointer out of fortran integer container

    // Copy velocity-traction array to GPU
    gpuMemcpy_todevice_realw(mp->veltrac1,veltrac1,(*size));
    gpuMemcpy_todevice_realw(mp->veltrac2,veltrac2,(*size));

}

extern EXTERN_LANG
void FC_FUNC_(interpolationparameter_to_gpu,
              INTERPOLATIONPARAMETER_TO_GPU)(long* Mesh_pointer,
                                                 realw* alip,
                                                 realw* blip) {
    TRACE("interpolationParameter_to_gpu");

    Mesh* mp = (Mesh*)(*Mesh_pointer); //get mesh pointer out of fortran integer container

    // Copy the interpolation factors to the mesh pointer
    mp->alip = *alip;
    mp->blip = *blip;
...
extern EXTERN_LANG
void FC_FUNC_(prepare_veltrac_gpu,
              PREPARE_VELTRAC_GPU)(long* Mesh_pointer, int* size) {

    TRACE("prepare_coupling_injection_gemini");

    Mesh* mp = (Mesh*)(*Mesh_pointer); //get mesh pointer out of fortran integer container

    // Allocate memory for the velocity-traction
    gpuMalloc_realw((void**)&(mp->veltrac1),(*size));
    gpuMalloc_realw((void**)&(mp->veltrac2),(*size));
}