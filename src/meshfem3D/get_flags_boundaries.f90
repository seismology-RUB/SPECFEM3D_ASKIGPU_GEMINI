...
!=====================================================================
  ...
  use constants
  use shared_parameters, only: MAP_TO_SPHERICAL_CHUNK     ! WF: mapping of cartesian box to spherical chunk
...
!
!  MD, WF: map back to cartesian box for followig checks
!
  if (MAP_TO_SPHERICAL_CHUNK) then
     do i = 1,8
        call mapSphericalChunkToCartesianBox(xelm(i),yelm(i),zelm(i))
     enddo
  endif
! ****************************************************
