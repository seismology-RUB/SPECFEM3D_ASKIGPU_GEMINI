...
! to compute the coordinate transformation
  ...
  double precision :: x,y,z      ! MD, WF: used for mapping to sphere
    ...
    do iy = 0,npy_element_steps
      do ix = 0,npx_element_steps
        ...
        do ir = 0,ner_layer(ilayer)
          ...
          if (MAP_TO_SPHERICAL_CHUNK) then     ! MD, WF: mapping of cartesian box to spherical chunk
             x = x_current
             y = y_current
             z = gamma*z_interface_top + (1.d0 - gamma)*z_interface_bottom
             call mapCartesianBoxToSphericalChunk(x,y,z)
             xgrid(ir + ioffset,ix,iy) = x
             ygrid(ir + ioffset,ix,iy) = y
             zgrid(ir + ioffset,ix,iy) = z
          else
