...
!=====================================================================
  ...
  use constants, only: IIN,MF_IN_DATA_FILES,MAX_STRING_LEN,IMAIN, &
    ...
    ILONGLAT2UTM,IGNORE_JUNK,DONT_IGNORE_JUNK,SMALLVAL
...
! local variables
  ...
  if (MAP_TO_SPHERICAL_CHUNK) then
     if (abs(LONGITUDE_MAX+LONGITUDE_MIN) > SMALLVAL) then
        stop 'Error: longitude limits should be symmetric'
     endif
     if (abs(LATITUDE_MAX+LATITUDE_MIN) > SMALLVAL) then
        stop 'Error: latitude limits should be symmetric'
     endif
  endif
